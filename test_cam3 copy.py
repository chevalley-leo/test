...
average_position_model = np.mean(initial_model_points, axis=0)
# Main loop
try:
    while True:
        ...
        rotated_pcd_model = pcd_model
        rotated_pcd_model.transform(initial_transformation)        
        rotated_pcd_model = rotated_pcd_model.voxel_down_sample(voxel_size)
        pcd_piece = pcd_piece.voxel_down_sample(voxel_size)
        vis.add_geometry(rotated_pcd_model)
        vis.add_geometry(pcd_piece)

        rot_Y = find_best_rotation(rotated_pcd_model, pcd_piece, axis='y', angle_range=(-30, 35))
        rot_X = find_best_rotation(rotated_pcd_model, pcd_piece, axis='x', angle_range=(-30, 35))
        rot_Z = find_best_rotation(rotated_pcd_model, pcd_piece, axis='z', angle_range=(0, 360))
